...
function mapDbTemplates(rows: Record<string, unknown>[]): HygieneCategory[] {
  ...
  const iconMap: Record<string, React.ReactNode> = {
    Housekeeping: <SprayCan className="h-5 w-5 text-teal-600" />,
    "Pest Control": <Bug className="h-5 w-5 text-orange-600" />,
  };

  return Array.from(categoryMap.entries()).map(([name, sections]) => ({
    name,
    icon: iconMap[name] ?? <SprayCan className="h-5 w-5 text-teal-600" />,
    sections,
  }));
}
...
const mockHygieneData: HygieneCategory[] = [
  {
    name: "Housekeeping",
    icon: <SprayCan className="h-5 w-5 text-teal-600" />,
    sections: [
      {
  ...
  return (
    <div className="flex items-center justify-between py-2.5 px-3 border-b border-slate-100 last:border-b-0 hover:bg-slate-50/50 transition-colors">
      ...
      <div className="flex items-center gap-1.5 shrink-0 ml-3">
        ...
        <button
          className="p-1.5 rounded-md hover:bg-teal-50 text-teal-500 transition-colors"
          title="View QR code"
          onClick={() => onView(template)}
        >
  ...
  return (
    <Card className="shadow-none border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-t border-slate-100">
        <button
          className="inline-flex items-center gap-1 px-3 py-1 text-[11px] text-teal-600 font-medium border border-teal-200 rounded-full hover:bg-teal-50 transition-colors"
          onClick={() => {
            const input = document.createElement("input");
            input.type = "file";
            input.accept = ".xlsx,.xls,.csv";
        ...
        {categories.map((category) => (
          <div key={category.name} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                className="flex items-center gap-1.5 text-[12px] text-teal-600 hover:text-teal-700 font-medium hover:underline"
                onClick={() =>
                  toast.success(
          ...
          <div className="space-y-3 pt-2">
            <div>
              ...
              <input
                ...
                className="w-full h-9 rounded-lg border border-slate-200 bg-white px-3 text-[13px] focus:outline-none focus:border-blue-300 focus:ring-1 focus:ring-teal-100"
              />
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button onClick={handleSaveEdit} className="h-9 text-[13px] rounded-lg bg-teal-600 hover:bg-teal-700 text-white">
                Save Changes
              </Button>
            </div>
          </div>
