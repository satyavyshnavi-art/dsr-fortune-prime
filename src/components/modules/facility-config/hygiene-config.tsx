...
function mapDbTemplates(rows: Record<string, unknown>[]): HygieneCategory[] {
  ...
  const iconMap: Record<string, React.ReactNode> = {
    Housekeeping: <SprayCan className="h-5 w-5 text-emerald-700" />,
    "Pest Control": <Bug className="h-5 w-5 text-orange-600" />,
  };

  return Array.from(categoryMap.entries()).map(([name, sections]) => ({
    name,
    icon: iconMap[name] ?? <SprayCan className="h-5 w-5 text-emerald-700" />,
    sections,
  }));
}
...
const mockHygieneData: HygieneCategory[] = [
  {
    name: "Housekeeping",
    icon: <SprayCan className="h-5 w-5 text-emerald-700" />,
    sections: [
      {
    ...
    <div className="col-span-1 lg:col-span-2 space-y-3 mt-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button className="h-7 text-[11px] px-2.5 bg-emerald-700 hover:bg-emerald-800 text-white" onClick={() => {
  ...
  return (
    <Card className="shadow-none border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-t border-slate-100">
        <button
          className="inline-flex items-center gap-1 px-3 py-1 text-[11px] text-emerald-700 font-medium border border-emerald-200 rounded-full hover:bg-emerald-50 transition-colors"
          onClick={() => {
            const input = document.createElement("input");
            input.type = "file";
            input.accept = ".xlsx,.xls,.csv";
        ...
        {categories.map((category) => (
          <div key={category.name} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                className="flex items-center gap-1.5 text-[12px] text-emerald-700 hover:text-emerald-800 font-medium hover:underline"
                onClick={() =>
                  toast.success(
          ...
          <div className="space-y-3 pt-2">
            <div>
              ...
              <input
                ...
                className="w-full h-9 rounded-lg border border-slate-200 bg-white px-3 text-[13px] focus:outline-none focus:border-blue-300 focus:ring-1 focus:ring-emerald-100"
              />
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button onClick={handleSaveEdit} className="h-9 text-[13px] rounded-lg bg-emerald-700 hover:bg-emerald-800 text-white">
                Save Changes
              </Button>
            </div>
          </div>
                ...
                <>
                  ...
                  <div className="space-y-5 pt-3">
                    ...
                    <div className="flex gap-3">
                      <Button
                        className="flex-1 h-10 text-[13px] bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg font-medium"
