...
function CreateUserDialog({ onCreated }: { onCreated?: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
      <DialogTrigger
        render={
          <Button size="sm" className="h-7 text-[11px] bg-violet-600 hover:bg-violet-700 text-white">
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form className="space-y-3" onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" size="sm" className="h-7 text-[11px] bg-violet-600 hover:bg-violet-700 text-white" disabled={loading}>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <button
            ...
            className="flex items-center gap-1 text-violet-600 font-medium text-[12px] hover:text-violet-800 cursor-pointer"
          >
            {row.original.role}
            <Pencil className="h-2.5 w-2.5 text-violet-400" />
          </button>
        ),
      },
      {
