...
export function PPMSchedule() {
  ...
  const getWeeksToShow = () => {
    switch (viewType) {
      case "day":
        // Show current week area (weeks 1-4)
        return Array.from({ length: 4 }, (_, i) => i + 1);
      case "week":
        return Array.from({ length: 12 }, (_, i) => i + 1);
      case "month":
        // Show ~4 weeks per "month" block, 6 months at a time
  ...
  // Toggle task status: scheduled/overdue -> completed, completed -> scheduled
  const handleToggleComplete = (taskId: string) => {
    setTasks((prev) =>
      prev.map((t) => {
        ...
        // scheduled, overdue, in_progress -> completed
  ...
  const handleAddPPM = () => {
    if (!newAsset || !newCategory || !newWeek) return;
    ...
    const newTask: PPMTask = {
      ...
      weekNumber: weekNum,
      status: "scheduled",
      scheduledDate: `${year}-01-01`, // Simplified
    };
          ...
          <div className="space-y-3 pt-1">
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button
                ...
                disabled={!newAsset || !newCategory || !newWeek}
